...
VPN_CONFIG=${VPN_CONFIG:-/opt/vpnserver/vpn_server.config}
VPNCMD="echo 1 | ${VPNCMD:-/opt/vpnserver/vpncmd}"
VPNSERVER=${VPNCMD:-//opt/vpnserver/vpnserver}

PSK=${PSK:-jedi}
USERNAME=${USERNAME:-jedi}
PASSWORD=${PASSWORD:-jedi}
...
if [ ! -f ${VPN_CONFIG} ]; then
  ...
  sudo ${VPNSERVER} start 2>&1 > /dev/null

  # while-loop to wait until server comes up
  # switch cipher
  while : ; do
    set +e
    ${VPNCMD} localhost /SERVER /CSV /CMD ServerCipherSet DHE-RSA-AES256-SHA 2>&1 > /dev/null
    [[ $? -eq 0 ]] && break
    set -e
    sleep 1
  done
  ...
  # enable L2TP_IPsec
  ${VPNCMD} localhost /SERVER /CSV /CMD IPsecEnable /L2TP:${IPSECENABLE_L2TP:-yes} /L2TPRAW:${IPSECENABLE_L2TPRAW:-yes} /ETHERIP:${IPSECENABLE_ETHERIP:-no} /PSK:${PSK} /DEFAULTHUB:DEFAULT

  # enable SecureNAT
  ${VPNCMD} localhost /SERVER /CSV /HUB:DEFAULT /CMD SecureNatEnable

  # fix DNS in the DHCP scope
  ${VPNCMD} localhost /SERVER /CSV /HUB:DEFAULT /CMD DhcpSet /START:${DHCPSET_START:-192.168.30.10} /END:${DHCPSET_END:-192.168.30.250} /MASK:${DHCPSET_MASK:-255.255.255.0} /EXPIRE:${DHCPSET_EXPIRE:-86400} /GW:${DHCPSET_GW:-192.168.30.1} /DNS:${DHCPSET_DNS:-8.8.8.8} /DNS2:${DHCPSET_DNS2:-8.8.4.4} /DOMAIN:${DHCPSET_LOCAL:-local} /LOG:${DHCPSET_LOG:-yes}
  ...
  if [[ "*${CERT}*" != "**" && "*${KEY}*" != "**" ]]; then
    ...
    ${VPNCMD} localhost /SERVER /CSV /CMD ServerCertSet /LOADCERT:server.crt /LOADKEY:server.key
    rm server.crt server.key
    export KEY='**'
  fi

  ${VPNCMD} localhost /SERVER /CSV /CMD OpenVpnMakeConfig openvpn.zip 2>&1 > /dev/null
  ...
  ${VPNCMD} localhost /SERVER /HUB:DEFAULT /CSV /CMD SetHubPassword ${HUB_PASSWORD}
  ...
  sudo ${VPNSERVER} stop 2>&1 > /dev/null
